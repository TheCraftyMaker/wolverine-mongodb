...
- **Escalation rule:** two non-obvious verification failures, or a broken plan assumption (an API that differs from the Verified Facts), means **stop and report**, then re-dispatch on Fable 5 with the failure context rather than improvising a different Wolverine API. For T1.1/T2.1/T4.5 specifically, if green cannot be reached after the listed levers, write up the generated code + failing facts and stop.
...
- [x] **Step 1:** Run the gated suite 5× per TFM under `RUN_MULTINODE`; record every result. **Result: 10/10 green (5× net9.0 + 5× net10.0), full `Category=multinode` suite, 17/17 each.**
- [x] **Step 2:** If 5× green both TFMs → remove the guard + add to CI; else document and keep the gate. **Un-gated: removed `#if RUN_MULTINODE`; `[Trait("Category","multinode")]` routes it into CI's existing multinode step (no ci.yml change needed).**
...
- [x] **Step 1:** Run all suites + pack + demo; record results.
- [x] **Step 2:** Confirm CI coverage; fix any gap; report. Commit only if CI changed.
