...
| **F12** | `fix/durability-agent-shutdown` | fix: durability agent StopAsync awaits its loops + disposes CTSes | **F4** | **Done** ([#196](https://github.com/TheCraftyMaker/wolverine-mongodb/pull/196)), 218 single-node + 18 multinode green on net9.0 and net10.0, zero edited facts | Sonnet |
...
| **F19** | `test/remediation-regression` | test: full cross-feature regression (inbox+outbox+saga+entity+solo+multinode) | **F6–F18 merged** | **Done** (report-only close, no PR), 218 single-node green on both TFMs; multinode 5× consecutive per TFM, 18/18 facts each (10/10 total); pack succeeds; demo suite 45/45 green; CI coverage confirmed adequate. Flagged a nuspec dependency-range question, resolved by F21 as a non-issue (already-documented decision, not a regression) | Sonnet |
| **F20** | *(no branch/PR)* | final verification on `main` (+ release decision) | **F15, F19, F21 complete** | Blocked by: F15, F19, F21 | Sonnet |
| **F21** | `docs/post-remediation-sweep` | docs: post-remediation documentation sweep | **F12, F19 complete** | **Done** | Sonnet |
...
**Status: done.** Branch `fix/durability-agent-shutdown`, PR [#196](https://github.com/TheCraftyMaker/wolverine-mongodb/pull/196). RED wasn't the loop-completion assertion
...
**One finding, investigated rather than fixed here (out of F19's verification-only scope):** the
packed nuspec shows plain pinned versions (`WolverineFx 6.21.0`, `MongoDB.Driver 3.10.0`), not F14's
bracketed ranges (`[6.16.0,7.0.0)` / `[3.9.0,4.0.0)`). At the time this session closed F19 it looked
like an unreviewed regression; **F21 re-investigated and found it is not one**: see F21's own notes
and the `FOLLOWUPS.md` entry it points to.
...
- **Goal:** Reconcile this plan doc, `FOLLOWUPS.md`, and the rest of the repo's docs with what F12/F19
  actually found and with current, re-verified reality, so F20 runs against accurate docs.
- **Scope:** Docs-only, no source/test/`Directory.Packages.props` changes. (1) Add the missing PR link to
  F12's row/status note (`#196`). (2) Land F19's status onto `main` (it existed only on the dangling,
  unmerged `test/remediation-regression` branch). (3) Re-verify (not copy) F19's flagged dependency-range
  finding: independently confirmed via `gh pr view` and `git log -p -- Directory.Packages.props` that
  dependabot PRs #162/#165/#166 touched only `demo/Directory.Packages.props`, never the root file, and
  that the root file's brackets were removed by a **deliberate** human PR
  ([#177](https://github.com/TheCraftyMaker/wolverine-mongodb/pull/177), "chore: one version per package +
  drop explicit SourceLink reference"), which already carries its own `CHANGELOG.md` entry and its own
  `FOLLOWUPS.md` entry ("Bounded NuGet dependency ranges, reversed by review decision (2026-07-26)")
  explaining the reversal. F19's "silently overwritten by dependabot" framing does not hold up; no new
  `FOLLOWUPS.md` entry was needed: the existing one already tells the accurate story. (4) Fact-check and
  fix drift in `CLAUDE.md`, `demo/CLAUDE.md`, `README.md`, `demo/README.md`, `CHANGELOG.md`,
  `CONTRIBUTING.md`, `SECURITY.md`, and `docs/upstream-contribution-notes.md` against current source/git
  history, found and fixed: README's status banner still read "pre-release (`0.1.0-beta.7`)" and named
  `WolverineFx 6.9.x` (stale since the 1.0.0 release on 2026-07-06 and the later 6.21.0 upgrade), plus an
  unverifiable "MongoDB.Driver 2.x also supported" claim; CONTRIBUTING.md's design notes still called
  `MongoDbUnitOfWork` "a planned follow-up enhancement" though it shipped in PR #50; SECURITY.md's
  supported-versions table still listed `0.1.x`; `CHANGELOG.md`'s `[Unreleased]` section was missing
  entries for F16 (dedup refactor, PR #187) and F18 (demo `CustomerFeedback` entity, PR #194). (5) Ran the
  humanizer skill over every doc touched, plus a style-only pass over the other dated
  `docs/superpowers/plans/*.md` design/discovery docs (no factual content changed there).
- **Expected output:** Docs match current, re-verified reality; plan doc and `FOLLOWUPS.md` current; PR merged.
- **Files/areas likely to change:** `docs/superpowers/plans/2026-07-07-review-findings-remediation.md`,
  `CLAUDE.md`, `README.md`, `CONTRIBUTING.md`, `SECURITY.md`, `CHANGELOG.md`, plus prose-only passes over
  other `docs/superpowers/plans/*.md` files.
- **Dependencies:** **F12, F19 complete.**
- **Blocking status:** **Done.**

- [x] **Step 1:** Add `#196` to F12's row and status note; land F19's status on `main`.
- [x] **Step 2:** Re-verify (not copy) the F19 dependency-range finding; correct the framing rather than
  file a redundant `FOLLOWUPS.md` entry once the existing one was confirmed accurate.
- [x] **Step 3:** Fact-check and fix Tier-1 docs; humanize Tier-1 docs and style-pass Tier-2 docs.
- [x] **Step 4:** Commit (`docs: post-remediation documentation sweep`), push, PR, checks green, update plan doc.
...
| R7 | **Version-range change vs dependabot/demo.** Bracketed ranges in `Directory.Packages.props` must not break dependabot PRs or the demo's separate solution. | F14 verifies a pack + restore locally and states the dependabot behavior in the PR; the demo pins `Wolverine.MongoDB 1.0.0` and has its own packages file, untouched. **Outcome:** dependabot never touched the root file's brackets (PRs #162/#165/#166 only bumped `demo/Directory.Packages.props`, confirmed via `gh pr view --json files`); the brackets were later removed by a deliberate human decision ([PR #177](https://github.com/TheCraftyMaker/wolverine-mongodb/pull/177), 2026-07-26) to unify package versions across the library and demo's separate dependabot ecosystems, documented in both `CHANGELOG.md` and a dated `FOLLOWUPS.md` entry. F19 flagged the plain-versioned nuspec as an open question; F21 traced it to this PR and closed it, not a regression, no separate fix needed. |
...
- **F19** ← F6-F18 merged (done, report-only close) · **F21** ← F12, F19 complete (done) · **F20** ← F15, F19, F21 complete
...
**Full-scope critical path:** `plan PR → F1 → F3 → F6 → F7 → F18 → F19 → F21 → F20` (8 sequential merges). The durability lane `F2 → F4 → {F8,F10,F11,F12} → F16/F17` is one merge shorter and runs **entirely in parallel** with the identity lane: it converges at F19. The small-fix lane (F9/F13/F14/F15) is depth 1 and never gates anything except F20 (F15 only). F21 turned out to be a short docs-only tail on F19, not a new lane.
